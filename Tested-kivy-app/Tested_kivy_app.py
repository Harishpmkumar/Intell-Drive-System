...
class SplashScreen(Screen):
    ...
    def show_buttons(self, dt):
        ...
        # Add the logo text
        logo_text = Label(text="Intelli Drive System for Detecting Road Obstacles", font_size=40,font_name="Candara", color=(1, 1, 1, 1), size_hint=(None, None), size=logo.texture_size, pos_hint={'center_x': 0.5, 'center_y': 0.9})
        main_screen.add_widget(logo_text)

        # Add the first two buttons
        button1 = HoverButton(text='Road Lane',font_size=30,font_name='Candara',size_hint=(.2, .2), pos_hint={'x': .2, 'y': .4}, md_bg_color=(0, 0.5, 0, 1))
        button1.bind(on_press=lambda btn, f='road_lane_detection.py': self.on_button_press(btn, f))
        main_screen.add_widget(button1)

        button2 = HoverButton(text='Pothole',font_size=30,font_name="Candara",size_hint=(.2, .2), pos_hint={'x': .6, 'y': .4}, md_bg_color=(0, 0.5, 0, 1))
        button2.bind(on_press=lambda btn, f='pothole_detection.py': self.on_button_press(btn, f))
        main_screen.add_widget(button2)

        button3 = HoverButton(text='Obstacles',font_size=30,font_name="Candara",size_hint=(.2, .2), pos_hint={'x': .2, 'y': .1}, md_bg_color=(0, 0.5, 0, 1))
        button3.bind(on_press=lambda btn, f='object_detection.py': self.on_button_press(btn, f))
        main_screen.add_widget(button3)

        button4 = HoverButton(text='Parking Space',font_size=30,font_name="Candara",size_hint=(.2, .2), pos_hint={'x': .6, 'y': .1}, md_bg_color=(0, 0.5, 0, 1))
